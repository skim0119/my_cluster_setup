...
class Globus(Storage):
    ...
    def retrieve(
        ...
        task_data.add_item(
            ...
            self.local_path(local_directory),
        )
        task_doc = self.transfer_client.submit_transfer(task_data)
        task_id = task_doc["task_id"]
        self.active_tasks.append(task_id)
...
if __name__ == "__main__":
    ...
    prepath = "zhidou2"
    name = "4_3_wiletype_mESC_motoneuron_spontaneous 1"
    globus.retrieve(remote_directory=os.path.join(prepath, name), local_directory=name)
